...
with tab1:
    ...
    st.info("Sube tu archivo Excel con las pestañas 'BaseQuery' y 'Activos' para ver las novedades diarias.")
...
with tab2:
    ...
    if 'df_base_general' in st.session_state:
        ...
        kpi_cols[1].metric("Altas del Período", '-' if len(df_altas_general) == 0 else len(df_altas_general))
        kpi_cols[2].metric("Bajas del Período", '-' if len(df_bajas_general) == 0 else len(df_bajas_general))
        if not df_desaparecidos.empty:
            kpi_cols[3].metric("Cambio Organizacional", len(df_desaparecidos))
    else:
        st.info("Sube un archivo en la pestaña 'Reporte Diario' para ver los resúmenes.")
    ...
    if archivo_para_periodo:
        try:
            ...
            if report_type == 'Semanal':
                ...
                dflt_start = today.replace(month=1, day=1); dflt_end = today.replace(month=12, day=31)
