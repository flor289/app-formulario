...
if uploaded_file is not None:
    try:
        ...
        # --- GENERACIÓN DE PDF (VERSIÓN FINAL Y ROBUSTA) ---
        def generar_pdf(datos_empleado):
            ...
            story = []
            story.append(Paragraph("PLAN DE DESARROLLO INDIVIDUAL (PDI)", styles['TituloPrincipal']))
            ...
            # --- FUNCIÓN AGREGAR_SECCION (REESCRITA PARA SER MÁS ROBUSTA) ---
            # Este cambio evita el error "too many values to unpack"
            def agregar_seccion(titulo, campos):
                bloque_seccion = [Paragraph(titulo, styles['TituloSeccion']), Spacer(1, 6)]
                # Iteramos de una forma más segura que no depende de "desempacar" valores
                for etiqueta in campos:
                    config = campos[etiqueta]
                    columna = config['col']
                    ...
                    if config.get('type') == 'checkbox':
                        ...
                        bloque_seccion.extend([Paragraph(f"<b>{etiqueta}:</b>", styles['Etiqueta']), Paragraph(format_as_list(valor), styles['NormalJustificado'])])
                    else:
                        bloque_seccion.extend([Paragraph(f"<b>{etiqueta}:</b>", styles['Etiqueta']), Paragraph(valor, styles['NormalJustificado'])])
                    bloque_seccion.append(Spacer(1, 10))
                
                story.append(KeepTogether(bloque_seccion))

            # --- Contenido del PDI ---
            # (El contenido y las llamadas a la función no cambian)
            ...
            data_tabla = [headers] + [["", "", "", "", "", ""] for i in range(4)]
